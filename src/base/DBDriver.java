...
public class DBDriver {
	...
	//	配置日志属性文件位置
	static String rootpath=System.getProperty("user.dir").replace(File.separator+"bin", "");
	static String logconf=rootpath+File.separator+"conf"+File.separator+"SOA"+File.separator+"logconf.properties";
	static String sysconf=rootpath+File.separator+"conf"+File.separator+"SOA"+File.separator+"Sys_config.xml";
	...
	/**[Function] 				删除指定的表中指定的记录或者清除表中所有记录
	 * [SQL命令]				DELETE FROM 表名称 WHERE 列名称 = 值;	
	 * [SQL命令]				TRUNCATE TABLE 表名称；			删除整个表的数据
	 * @param DBname	待读取的表名
	 * @param filter			过滤器
	 * @throws  				404 - 没有找到要删除的记录
	 * @throws  				500 - 数据库故障
	 */
	public void DelSQla(String DBname,String filter) throws Throwable{
		String Sqlcom="";
		try {
			PreparedStatement pstt=null;
			Connection conn=connectSQL("db");
			if(conn.isClosed())throw new SQLException("[info]500,数据库连接错误！");				
			Sqlcom="delete from "+DBname+" where "+filter+";";
			pstt = conn.prepareStatement(Sqlcom);
			PropertyConfigurator.configure(logconf);
			logger.info("SQL cmd: "+Sqlcom);
			pstt.executeUpdate();
	 ...
	 * @throws  				500 - 数据库故障
	 */
	public int UpdateSQla(String DBname,String filter,String Colname,String newValue) throws Throwable{
		int rs=0;
		try {
			Connection conn=connectSQL("db");
			if(conn.isClosed())throw new SQLException("[info]500,数据库连接错误！");	
			String Sqlcom="UPDATE "+DBname + " SET "+Colname+"='"+ newValue + "' WHERE " + filter+";";
			PropertyConfigurator.configure(logconf);
			logger.info("SQL cmd: "+Sqlcom);
			PreparedStatement pst = conn.prepareStatement(Sqlcom);
			rs=pst.executeUpdate();	//返回0表示没有记录被更新
